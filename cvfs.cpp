...
void man(char *name)
{
    ...
    {
        printf("Description: used to data from rgular file\n");
        printf("Usage:read File_Name No_of_Bytes_To_Read\n");
    }
    ...
    {
        printf("ERROR: No manual entry avialable.\n");
    }
}
...
void DisplayHelp()
{
   ...
   printf("write: To write content into the file ");
   printf("exit:To terminate the file system\n");
   printf("stat:To Display information of file using name\n");
   printf("fstat:To Display information of file using file decsriptor\n");
...
(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)=(UFDTArr->ptrfiletable->ptrinode->FileActualSize)+isize;
     ...
     else if(count==2)
     {
        ...
        else if(strcmp(command[0],"fstat")==0)
        {
            ...
            if(ret==-1)
                 printf("ERROR:Incorrect parameters\n");
            if(ret=2)
                 printf("ERRor: There is nosuch file\n");
            continue;        
        }
        ...
        else if(strcmp(command[0],"write")==0)
        {
            ...
            if(ret==-2)
                  printf("ERROR:There is no such sufficient memory to write\n");
            if(ret==-3)
        ...
        else if(count==3)
        {
            if(strcmp(command[0],"create")==0)
            {
                ...
                if(ret>=0)
                      printf("File is sucessfully creted with file descriptor:%d\n",ret);
                if(ret==-1)
                     printf("ERROR: incorrect parameter\n");
                if(ret==-2)
        ...
        else if(strcmp(command[0],"open")==0)
        {
            ...
            if(ret>=0)
                 printf("File is sucessfully opened with file descriptor:%d\n",ret);
            if(ret==-1)
                 printf("ERROR: Incorrect parameters\n");
            if(ret==-2)
                 printf("ERROR:File not present\n");
            if(ret=-3)
                 printf("ERROR: Permission is denied \n");
             continue;               
        }
